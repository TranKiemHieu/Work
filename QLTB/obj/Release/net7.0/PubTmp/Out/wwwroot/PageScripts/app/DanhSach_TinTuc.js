...
$(document).ready(function () {
    ...
    Promise.all([loadDanhSach(), loadHoatDongDoanThe(), loadTinChuyenNganh(), loadBaiVietKhac()])
        .then(afterDataLoaded)
        .catch(error => console.error('Error loading data:', error));

    function loadDanhSach() {
        $('#section-list-content').empty();
        $.ajax({
            type: "POST",
            url: "/BaiViet/_ViewList",
            data: {
                
                "viewName": "_section_list_content",
                "maViewCapCha": 149,
                "urlChuyenMuc": urlChuyenMuc
            },
            success: function (response) {
                console.log('response', response)
                $('#section-list-content').html(response)
            }
        });
    }

    function loadHoatDongDoanThe(pageIndex = 1) {
        $('#section-sidebar-1').empty();
        $.ajax({
            type: "POST",
            url: "/BaiViet/_View",
            data: {
                "pageIndex": pageIndex,
                "viewName": '_section_sidebar_1',
                "maViewCapCha": 149,
                "sessionName": ''
            },
            success: function (response) {
                $('#section-sidebar-1').html(response);
    ...
    function loadTinChuyenNganh(pageIndex = 1) {
        $('#section-sidebar-2').empty();
        $.ajax({
            type: "POST",
            url: "/BaiViet/_View",
            data: {
                "pageIndex": pageIndex,
                "viewName": '_section_sidebar_2',
                "maViewCapCha": 149,
                "sessionName": ''
            },
            success: function (response) {
                $('#section-sidebar-2').html(response);
                if (totalRow != undefined && pageSize != undefined) {
