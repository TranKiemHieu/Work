...
namespace QLTB.Repository
{
    public class TinTucRepository : ITinTucRepository
    {
        ...
        public async Task<Result<List<TB_BaiViet>>> GetBaiVietByURLChuyenMuc(string URLChuyenMuc)
        {
            try
            {
                using (var conn = _connectDB.IConnectData())
                {
                    ...
                    var result = await conn.QueryAsync<TB_BaiViet>(new CommandDefinition(sp, parameters, commandType: System.Data.CommandType.StoredProcedure));
                    //xử lý kết quả trả trước khi trả về (nếu có)
                    if (result == null)
                        return Result<List<TB_BaiViet>>.Failure("Không tìm thấy bài viết.");
